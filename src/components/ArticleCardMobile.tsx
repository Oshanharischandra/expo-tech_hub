...
const ArticleCardMobile: React.FC<ArticleCardMobileProps> = ({ article, index = 0 }) => {
    ...
    return (
        <GlowCard customSize glowColor="gold" className="rounded-xl overflow-hidden mb-4">
            <motion.article
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: index * 0.05 }}
                className="bg-[#141414]/90 rounded-xl overflow-hidden active:scale-[0.99] transition-transform"
            >
                        ...
                        <div className="flex-1 min-w-0 p-4 space-y-2">
                            ...
                            <div className="flex items-center gap-2">
                                {article.customAuthor ? (
                                    <>
                                        <div className="w-5 h-5 rounded-full bg-dark-800 flex items-center justify-center text-[8px] font-bold text-gray-400 border border-[#ac834e]/30">
                                    ...
                                    <>
                                        {article.author.avatar ? (
                                            <img src={article.author.avatar} alt="" className="w-5 h-5 rounded-full border border-[#ac834e]/30" />
                                        ) : (
                                            <div className="w-5 h-5 rounded-full bg-dark-800 flex items-center justify-center text-[8px] font-bold text-gray-400 border border-[#ac834e]/30">
                            ...
                            <div className="flex items-center gap-4 text-xs text-gray-500 pt-1">
                                <span className="flex items-center gap-1">
                                    <Clock className="w-3 h-3 text-[#ac834e]" /> {article.readingTime}m
                                </span>
                                <span className="flex items-center gap-1">
                                    <Heart className="w-3 h-3 text-[#ac834e]" /> {article.likes}
                                </span>
                                <span className="flex items-center gap-1">
                                    <MessageCircle className="w-3 h-3 text-[#ac834e]" /> {article.comments.length}
                                </span>
                            </div>
                        </div>
                        {/* Image */}
